-- ATUALIZAÇÃO DEFICIENCIAS
with deficiencias_atual as (
select 
ci_aluno,
...
left join academico.tb_aluno_recurso_espec_aval ar on ar.cd_aluno = ta2.ci_aluno -- em 10/10/2022
where exists (select 1 from public.tb_spaece_em_aplicacao_2022 e where ci_aluno = e.cd_aluno_sige ) --and cd_deficiencia is not null
group by 1,2
)
select 
orgao_regional,
...
tp_etapa_ensino_turma::INT,
ds_modalidade,
nm_curso_profissional,
ds_etapa_serie,
tp_etapa_ensino::INT,
cd_aluno_sige::BIGINT,
...
cd_cor_raca::INT,
ds_cor_raca,
da.ds_possui_deficiencia,
da.nm_deficiencia,
nm_aluno,
nm_mae,
nm_pai--,
da.ds_recurso_avaliacao
from public.tb_spaece_em_aplicacao_2022 e
left join deficiencias_atual da on ci_aluno = cd_aluno_sige
--where not exists (select 1 from deficiencias_atual da where ci_aluno = cd_aluno_sige)
...
and (tt.cd_etapa=214 or tt.cd_anofinaleja=2) 
