...
ult_ent_origem as (
  select *
  from public.tb_spaece_em_aplicacao_2022 tsea 
...
*
 ...
 where exists (select 1 from ult_ent_origem eo where eo.cd_aluno_sige = tu.cd_aluno )
 and fl_tipo_atividade <> 'AC'
 and tu.nr_anoletivo = 2022
  ),
  relatorio as (
  
 select
 eo.cd_aluno_sige "CD_ALUNO_SIGE",
...
case when (tor.cd_etapa=214 or tor.cd_anofinaleja=2) then 72 
      ...
      when tor.cd_etapa = 186 then 32 end "TP_ETAPA_ENSINO_ORIGEM", 
...
case when tde.cd_modalidade in (36,40) then 'Regular'
 ...
 left join ult_ent_destino ed on eo.cd_aluno_sige = ed.cd_aluno
 join academico.tb_turma tor on tor.ci_turma = eo.id_turma and tor.nr_anoletivo = 2022
 left join academico.tb_turma tde on tde.ci_turma = ed.cd_turma and tde.nr_anoletivo = 2022
...
*
 from relatorio 
 where 
 "ID_ESCOLA_ORIGEM" <> "ID_ESCOLA_DESTINO" 
