...
tt.ds_ofertaitem,
tn.ds_turno,
tt.cd_etapa cd_etapa_oferta,
ds_etapa ds_etapa_oferta,
tc.nm_curso,
cd_nivel,
ds_nivel,
case 
    when cd_etapa in (213,214,195,194,175,196,174,173) or cd_modalidade = 38  then 1
    when cd_etapa in (121,122,123,124,125,126,127,128,129,183,162,184,188,163,185,189,164,186,190,165,187,191,180,181)
                                                               then 0 else 99 end fl_eja,
case when tc.fl_possui_estagio = 'S' then 1 else 0 end fl_estagio_curso
from academico.tb_turma tt 
join academico.tb_turno tn on tn.ci_turno = tt.cd_turno 
join academico.tb_etapa et on et.ci_etapa = cd_etapa 
join academico.tb_modalidade m on m.ci_modalidade = tt.cd_modalidade 
join academico.tb_nivel on ci_nivel =  cd_nivel
left join academico.tb_curso tc on ci_curso = cd_curso 
where
...
case when cd_etapa in (121,122,123,124,125,126,127,128,129,183,162,184,188,163,185,189,164,186,190,165,187,191,180,181)
                                                               then 'Regular'
       when cd_etapa in (213,214,195,194,175,196,174,173) or cd_modalidade = 38  then 'EJA' else 'OUTRO' end ds_modalidade_oferta,
ae.cd_nivel,
tn.ds_nivel,
...
select --distinct ds_ofertaitem from turma  where  ds_modalidade_oferta = 'OUTRO'
cb.nr_anoletivo,
cb.cd_crede_sefor,
cb.nm_crede_sefor,
cb.id_municipio,
...
cb.id_escola_inep,
cb.nm_escola,
cb.nm_localizacao_zona,
cb.fl_sede,
cb.nm_local_funcionamento,
cb.cd_tipo_local,
cb.nm_tipo_local,
cb.cd_turma,
tt.ds_turma,
...
tt.cd_nivel,
tt.ds_nivel,
cb.cd_etapa cd_etapa_aluno,
cb.ds_etapa ds_etapa_aluno,
cb.cd_aluno,
cb.nm_aluno,
cb.nm_social,
cb.dt_nascimento,
cb.ds_raca,
cb.nm_etnia_indigena,
cb.ds_sexo,
cb.nr_idade,
cb.fl_possui_cpf,
cb.fl_possui_bolsa_escola,
cb.fl_possui_nis,
cb.nr_cpf,
cb.nr_identificacao_social,
cb.fl_transporte,
cb.fl_possui_deficiencia,
cb.fl_cegueira,
cb.fl_baixa_visao,
cb.fl_surdez,
cb.fl_deficiencia_auditiva,
cb.fl_surdocegueira,
cb.fl_deficiencia_fisica,
cb.fl_deficiencia_intelectual,
cb.fl_deficiencia_multipla,
cb.fl_autismo,
cb.fl_altas_habilidades_superdotacao,
cb.fl_depressao,
cb.fl_discalculia,
cb.fl_dislalia,
cb.fl_dislexia,
cb.fl_epilepsia,
cb.fl_esquizofrenia,
cb.fl_síndrome_de_down,
cb.fl_síndrome_do_panico,
cb.fl_transtorno_opositor_desafiador,
cb.fl_tdah,
cb.fl_visao_monocular
