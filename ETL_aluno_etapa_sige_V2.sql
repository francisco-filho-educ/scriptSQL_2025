create table public.tb_dm_etapa_aluno as (
with turma as (
select *
...
case when cd_etapa =   180 then 1 -- CRECHE
     when cd_etapa =   181 then 2 -- PRE-ESCOLA
     when cd_etapa in (121,122,123,124,125,194)   then 3 --ANOS INICIAIS
     when cd_etapa in (126,127,128,129,174,195)   then 4 -- ANOS FINAIS
     when cd_etapa  = 175 then 5 --ANOS INICIAIS E FINAIS - EJA
     else 99 end cd_etapa_fase_aluno,
case 
...
max(case when tad.cd_deficiencia = 13 then 1 else 0 end) fl_super_dot
