...
and exists (select 1 from rede_fisica.tb_unidade_trabalho tut where tut.ci_unidade_trabalho = tt.cd_unidade_trabalho 
                  and tut.cd_dependencia_administrativa = 2 --and tut.ci_unidade_trabalho = 36
                  )
...
from turma_disc td
left join notas using(cd_turma,cd_periodo,cd_disciplina)
group by 1,2,3,4
) 
,
alunos_abaixo as (
...
nr_abaixo
from turma t 
join enturmados et on et.cd_turma = t.ci_turma
join alunos a on a.ci_aluno = et.cd_aluno 
join turma_periodo td on et.cd_turma = td.cd_turma  
left join alunos_abaixo ab on ab.cd_turma = et.cd_turma and ab.cd_aluno = et.cd_aluno and td.cd_periodo = ab.cd_periodo  --8864 --8935
) 
