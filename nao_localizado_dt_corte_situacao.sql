...
and exists(select 1 from public.tb_dm_etapa_aluno_2023_03_21 tdea where tm.cd_aluno = tdea.cd_aluno)
and not exists(select 1 from academico.tb_ultimaenturmacao tu where nr_anoletivo = 2023 and tu.cd_aluno = tm.cd_aluno and tu.fl_tipo_atividade <>'AC')
group by 1
        )      
, mov_oferta as( 
select
