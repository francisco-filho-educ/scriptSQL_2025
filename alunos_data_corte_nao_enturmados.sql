...
and cd_nivel = 27 
--and tt.cd_modalidade <> 38 --and tt.cd_etapa in (162,163,164)
and exists (select 1 from rede_fisica.tb_unidade_trabalho tut where tut.ci_unidade_trabalho = tt.cd_unidade_trabalho and tut.cd_categoria = 8)

)
,ultima_enturmacao as (
select 
cd_aluno,
cd_turma
from academico.tb_ultimaenturmacao tu 
where exists (select 1 from turmas where cd_turma = ci_turma) --and cd_aluno = 234765
)
--select *  from ultima_enturmacao
,localizar as (
select 
teed.cd_aluno
from tmp.tb_enturmados_eeep_dt teed 
where not exists (select 1 from ultima_enturmacao ut where ut.cd_aluno = teed.cd_aluno)
) 
--select count(1) from localizar --58943
,ultimo_mov as (
select 
cd_aluno,
...
and exists(select 1 from localizar tu where  tu.cd_aluno = tm.cd_aluno )
group by 1
        )      
, mov_oferta as( 
select
...
select  --nm_crede_sefor , nm_escola from escolas join turmas t on t.cd_unidade_trabalho = id_escola_sige group by 1,2
nr_anoletivo,
id_crede_sefor, 
nm_crede_sefor, 
nm_municipio,
...
ds_motivo
from mov_oferta ao
join escolas on cd_unidade_trabalho_destino = id_escola_sige --group by 1,2,3,4
