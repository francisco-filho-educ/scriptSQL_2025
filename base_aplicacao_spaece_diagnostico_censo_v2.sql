...
upper(tet.ds_etapa_ensino) "DC_ETAPA",
...
'' "NM_ANEXO",
count(1) "QT_ALUNO",
sum(case when IN_RECURSO_BRAILLE = 1 then 1 else 0 end) "QT_ALUNO_BRAILE",
sum(case when IN_RECURSO_AMPLIADA_18  = 1  or IN_RECURSO_AMPLIADA_24 = 1 then 1 else 0 end) "QT_ALUNO_AMPLIADA",
sum(case when IN_RECURSO_LEDOR = 1 then 1 else 0 end) "QT_ALUNO_LEDOR",
sum(case when tm.in_surdez = 1 then 1 else 0 end) "QT_ALUNO_SURDEZ",
